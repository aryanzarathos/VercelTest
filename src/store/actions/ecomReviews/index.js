...
export const postReviewDetails = (id, data, type) => {
  ...
  return (dispatch) => {
    ...
    EcomReviewsRequest.post(
      EcomReviewsRequest.EcomCurrencyEndpoint.postReview.replace(
        "_BUSINESS_",
        id
      ),
      data,
      (success) => {
        // console.log("line 27 review",success)
        dispatch({
        ...
        if (type === "user") {
          ...
          dispatch(showToast("Review Posted!"));
        }
      },
      (error) => {
...
export const getAllReviewList = (id, sort, status, search, limit, page) => {
  return (dispatch) => {
    ...
    if (limit) {
      EcomReviewsRequest.get(
        EcomReviewsRequest.EcomCurrencyEndpoint.getReviews
          .replace("_BUSINESS_", id)
          .replace("_SORT_", sort)
          .replace("_STATUS_", status)
...
export const getSingleReview = (id) => {
  return (dispatch) => {
    ...
    EcomReviewsRequest.get(
      EcomReviewsRequest.EcomCurrencyEndpoint.getReviewData.replace(
        "_REVIEW_",
        id
      ),
      (success) => {
...
export const patchReviewDetails = (id, data) => {
  return (dispatch) => {
    ...
    EcomReviewsRequest.patch(
      EcomReviewsRequest.EcomCurrencyEndpoint.patchReview.replace(
        "_BUSINESS_",
        id
      ),
      data,
...
export const deleteReview = (businessid, id) => {
  return (dispatch) => {
    ...
    EcomReviewsRequest.delete(
      EcomReviewsRequest.EcomCurrencyEndpoint.deleteReview
        .replace("_BUSINESS_", businessid)
        .replace("_REVIEW_", id),
      (success) => {
        dispatch({
...
export const multipleDeleteReviewDetails = (id, data) => {
  return (dispatch) => {
    ...
    EcomReviewsRequest.patch(
      EcomReviewsRequest.EcomCurrencyEndpoint.multipleDeleteReview.replace(
        "_BUSINESS_",
        id
      ),
      data,
...
export const readUnreadReviewReplies = (id, reviewID) => {
  return (dispatch) => {
    EcomReviewsRequest.patch(
      EcomReviewsRequest.EcomCurrencyEndpoint.readUnReadReview
        .replace("_BUSINESS_", id)
...
export const replyStatusUpdate = (id, reviewID, data) => {
  return (dispatch) => {
    EcomReviewsRequest.patch(
      EcomReviewsRequest.EcomCurrencyEndpoint.patchReplyStatus
        .replace("_BUSINESS_", id)
        .replace("_REVIEW_", reviewID),
      data,
      (success) => {
...
export const postReviewReply = (id, reviewID, data, type, template) => {
  return (dispatch) => {
    ...
    EcomReviewsRequest.post(
      EcomReviewsRequest.EcomCurrencyEndpoint.postReply
        .replace("_BUSINESS_", id)
        .replace("_REVIEW_", reviewID),
      data,
      (success) => {
...
export const deleteReviewReply = (businessid, reviewid, replyId) => {
  return (dispatch) => {
    ...
    EcomReviewsRequest.delete(
      EcomReviewsRequest.EcomCurrencyEndpoint.deleteReply
        .replace("_BUSINESS_", businessid)
        .replace("_REVIEW_", reviewid)
        .replace("_REPLY_", replyId),
      (success) => {
        dispatch({
...
export const reviewStatusUpdate = (id, reviewID, status, type) => {
  return (dispatch) => {
    EcomReviewsRequest.patch(
      EcomReviewsRequest.EcomCurrencyEndpoint.patchReviewStatus
        .replace("_BUSINESS_", id)
        .replace("_REVIEW_", reviewID)
        .replace("_STATUS_", status),
      {},
      (success) => {
        ...
        if (status === "Published") {
          dispatch(
            showToast(
              "Review Approved. See approved Reviews in Approved menu"
            )
          );
        } else {
          dispatch(
            showToast(
              "Review Rejected. See rejected Reviews in Rejected menu", "", "error"
            )
          );
        }
      },
...
export const multiplePermanantDeleteReview = (id, data) => {
  return (dispatch) => {
    EcomReviewsRequest.patch(
      EcomReviewsRequest.EcomCurrencyEndpoint.multiplePermanantDelete.replace(
        "_BUSINESS_",
        id
      ),
      data,
...
export const restoreMultipleReviews = (id, data) => {
  return (dispatch) => {
    EcomReviewsRequest.patch(
      EcomReviewsRequest.EcomCurrencyEndpoint.multipleRestoreReview.replace(
        "_BUSINESS_",
        id
      ),
      data,
      (success) => {
        ...
        dispatch(showToast('Restore Successfully!'));
      },
      (error) => {
        // dispatch(setCommonError(error.message));
      }
...
export const getProductReviewList = (id) => {
  return (dispatch) => {
    ...
    EcomReviewsRequest.get(
      EcomReviewsRequest.EcomCurrencyEndpoint.getProductReviews.replace(
        "_PRODID_",
        id
      ),
      (success) => {
...
export const getBuyerProductReviewDetails = (id, prodId, userId) => {
  return (dispatch) => {
    EcomReviewsRequest.get(
      EcomReviewsRequest.EcomCurrencyEndpoint.getBuyerProductRelation
