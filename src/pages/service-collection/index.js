...
import { serviceCollectionfetchTemplateData } from "../../CommonFunctions/fetchTemplateData";
...
const Home = ({ isPrivateDomain, subdomain, preloadedState }) => {
    ...
    return (
      <Fragment>
        ...
        />
        <FontLoader
          headerFont={preloadedState?.template?.draftfontfamily?.header}
          paragraphFont={preloadedState?.template?.draftfontfamily?.paragraph}
        />

        {preloadedState?.upper?.length
          ? preloadedState?.upper?.map((item, key) => {
              // console.log(item, "item2")
              return (
                <div key={key}>
                  {item?.container_name === "Top Header"
                    ? preloadedState?.hide_topheader === false &&
                      typeof RenderComponent[item.componentRender] !==
                        "undefined"
                      ? React.createElement(
                      ...
                      : ""
                    : preloadedState?.hide_header === false &&
                      typeof RenderComponent[item.componentRender] !==
                        "undefined"
                    ? React.createElement(
                        RenderComponent[item.componentRender],
          ...
          : ""}
        <React.Fragment>
          {preloadedState?.data?.length > 0 ? (
            preloadedState?.data?.map((item, key) => {
              return (
        ...
        {preloadedState?.lower?.length
          ? preloadedState?.lower?.map((item, key) => {
              return (
                <div key={key}>
                  {typeof RenderComponent[item.componentRender] !== "undefined"
                    ? React.createElement(
...
export async function getServerSideProps({ req }, context) {
    let resData = await serviceCollectionfetchTemplateData(req.headers.host, req.url)
