...
import { productDetailsfetchTemplateData } from "../../CommonFunctions/fetchTemplateData";
    ...
    if (globalTheme) {
      for (let globalcssitem in globalTheme?.color) {
        root.style.setProperty(
          `--${globalcssitem}`,
          globalTheme?.color[globalcssitem]
        );
      }
      let types = [
        "typoAnchor",
      ...
      for (let type of types) {
        for (let globalcssitem in globalFonts?.[type]) {
          ...
          root.style.setProperty(
            `--${globalcssitem}`,
            globalFonts?.[type][globalcssitem]
          );
        }
      }
    }
...
export async function getServerSideProps({ req, query, params }, context) {
  ...
  let resData = await productDetailsfetchTemplateData(req.headers.host, slug);
