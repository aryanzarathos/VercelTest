...
import { hostName, serverSideHost } from "../../CommonFunctions/isBrowser";
...
import { LoginSignupfetchTemplateData } from "../../CommonFunctions/fetchTemplateData";
...
export async function getServerSideProps({ req }, context) {
  ...
  const cookieValue = req.headers.cookie
    ? req.headers.cookie.split(';').find(cookie => cookie.trim().startsWith(`${parts[0]}=`))
    : null;
  ...
  if (cookieValue) {
    return {
      redirect: {
        destination: '/', // Replace with the URL you want to redirect to
        permanent: false,
      },
    };
  }
  let resData = await LoginSignupfetchTemplateData(serverSideHost(req.headers.host), req.url)
