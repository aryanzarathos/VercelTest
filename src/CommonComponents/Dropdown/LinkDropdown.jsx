...
const LinkDropdown = ({ ModalsRef, onClose }) => {
    ...
    if (isDetailsValid && Valid) {
      ...
      dispatch(
        UPDATE_BUTTON_DATA({
          ...
          data: updatedData[selectedElementIndex],
        })
      );
  ...
  useEffect(() => {
    if (selectedElementData) {
      let data = selectedElementData?.pathname?.split("/");
      setButtonName(selectedElementData?.text || "");
      setEmail(selectedElementData?.email || "");

      setPathname(selectedElementData?.pathname?.includes("product-details") ? "product-details" : selectedElementData?.pathname || "");
      setURLSlug(data && data.length ? data[1] : "")
      setWebUrl(selectedElementData?.webUrl || "");
      setTelnumber(selectedElementData?.telnumber || "");
      setAction(selectedElementData?.action || "");
      setFile(selectedElementData?.file || "");
      setScrollTo(selectedElementData?.scrollTo || "");
    }
  }, [selectedElementData]);
