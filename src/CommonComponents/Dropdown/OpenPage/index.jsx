...
const ProductPageSelectDropDown = ({ values,NavigationValues, handleSelect,label, showValue, disabled,deafultValue ,className,selectClassName,selectListClassName}) => {
  ...
  return (
    <div className={`${styles.selectDropDownContainer} ${className}`  } ref={clickOutsideRef} aria-disabled={disabled}>
      ...
      {
        openDropdown &&
        <SelectdropDownContainer selectListClassName={selectListClassName} NavigationValues={NavigationValues} values={values} onChange={(val,title)=>handleOnChange(val,title)} />
      }
    </div>
  )
}

export default ProductPageSelectDropDown