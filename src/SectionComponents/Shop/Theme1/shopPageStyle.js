...
`;
export const SortBy = styled.button`
...
export const RibbonMain = styled.div`
  position: ${({ item }) => item?.desktop?.position};
  ...
  max-width: ${({ item }) => item?.desktop?.maxWidth};
  border-top-right-radius: ${({ item }) => item?.desktop?.borderTopRightRadius};
  border-bottom-right-radius: ${({ item }) => item?.desktop?.borderBottomRightRadius};
...
export const PopularListItem = styled.li`
  ...
  svg{
      path{
        stroke: ${({item})=>item?.desktop?.active?.strokeColor ? item?.desktop?.active?.strokeColor : "#000"};
        stroke-opacity: 1;
      }
    }
    }
