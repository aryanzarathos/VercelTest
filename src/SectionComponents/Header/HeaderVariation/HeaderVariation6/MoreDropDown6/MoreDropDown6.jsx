...
import { MoreContainer, MoreList, MoreListItem, MoreSubLink, MoreSubLinkSection, MoreSubListItem } from './MoreDropDown6.style'
...
import { SubSubLinkSection } from '../HeaderDropDown6/dropDown6.style';
import { UserContext6 } from '..';
import { useRouter } from 'next/router';
import ValidationFile from '@/Classes/ValidationFile';

const MoreDropDown6 = ({ DropDownData, dropDownStyle }) => {
                    ...
                    selecetedList === "1" ? (
                        <MoreList item={style?.MoreList}>
                            <MoreSubListItem className='border' item={style?.MoreSubListItem}
                                ...
                                <Button onClick={() => RedirectTo(dropDownData?.page?.page_slug, dropDownData?.page?.prefix, dropDownData?.page)}
                                                    ...
                                                    (subItemExpand === item._id) && item.data && item.data.length > 0 && item.data.map((subItem, subSubKey) => (
                                                        <>
                                                            <SubSubLinkSection item={style?.SubSubLinkSection} subSubKey={subSubKey}>
                                                                <Button className={item?.page?.page_slug === pathname ? 'active' : ''} text={subItem?.page.page_name} style={SubSubLinkSectionBtn}
