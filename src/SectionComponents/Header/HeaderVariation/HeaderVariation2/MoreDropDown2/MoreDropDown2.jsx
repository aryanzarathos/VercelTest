...
import { SubSubLinkSection } from "../HeaderDropDown2/dropDown2.style";
import { UserContext2 } from "..";
import { useRouter } from "next/router";
import ValidationFile from "@/Classes/ValidationFile";

const MoreDropDown2 = forwardRef(({ DropDownData, dropDownStyle }, ref) => {
  const styles = useContext(UserContext2)
  ...
  // console.log("more", dropDownStyle)
  ...
  return (
    <MoreContainer item={style.MoreContainer} ref={ref}>
      {selecetedList === "1" ? (
        <MoreList item={style?.MoreList}>
          <MoreSubListItem
            className="border"
            item={style?.MoreSubListItem}
            ...
            <>
              {dropDownData.data.map((item, key) => {
                return (
                  <MoreListItem
                    key={key}
                    item={style?.MoreListItem}
                    itemDownSvg={style?.DownIcon}
                  >
                    <Button
                      onClick={() =>
                        RedirectTo(
                    ...
                    {subItemExpand === item._id &&
                      item.data &&
                      item.data.length > 0 &&
                      item.data.map((subItem, subSubKey) => (
                        <>
                          <SubSubLinkSection
                            item={style?.SubSubLinkSection}
                            subSubKey={subSubKey}
                          >
                            <Button
                              text={subItem?.page.page_name}
                              style={SubSubLinkSectionBtn}
                              className={item?.page?.page_slug === pathname ? 'active' : ''}
                              onClick={() =>
                                RedirectTo(
              ...
              {DropDownData.map((item, key) => {
                return (
                  ...
                  >
                    ...
                    {item?.data && item.data.length > 0 ? (
                      <Button
                        onClick={() => OpenMoreInnerDropDown(item)}
                        style={MoreSubListItemBtn}
                      >
                        <MoreRightArrow />
                      </Button>
...
MoreDropDown2.displayName = "MoreDropDown2";
export default MoreDropDown2;
