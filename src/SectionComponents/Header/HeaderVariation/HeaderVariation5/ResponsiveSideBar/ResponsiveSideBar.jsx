...
import ResponsiveBookingButton from './ResponsiveBookingButton/ResponsiveBookingButton';
import { useRouter } from 'next/router';
import ValidationFile from '@/Classes/ValidationFile';

const ResponsiveSideBar = forwardRef(({ show, setShow, data, FinalData, ResponsiveBooking, bookingAppointmentButton, AppointmentRedirect }, ref) => {
    ...
    let styles = {
        ...
        ResponsiveBookAppointment: data?.custom_section_editable[55],

    }
    let responsiveBookingAppointmentButton = {
        ...data?.custom_section_editable[55],
        text: "Booking Appointment",
        styles: {
            ...data?.custom_section_editable[55]?.style,
            desktop: {
                ...data?.custom_section_editable[55]?.style?.desktop,
                "color": "#fff",
                "backgroundColor": "rgba(31, 50, 89,.8)",
                hover: {
                    ...data?.custom_section_editable[55]?.style?.desktop?.hover,
                    "color": "#fff",
                    "backgroundColor": "rgba(31, 50, 89,1)"
                },
                active: {
                    ...data?.custom_section_editable[55]?.style?.desktop?.hover?.active,
                    "color": "white",
    ...
    return (
        ...
        <SideNavbarWrapper ref={ref} item={styles?.SideNavbarWrapper}
            ...
            <UserProfile item={styles?.UserProfile}>
                ...
                {(AppLinkUrl.subdomain() && Auth.subdomainUser()) ||
                    (AppLinkUrl.privateDomain() && Auth.user()) ?
                    <Text tagType={styleData.custom_section_editable[11].tagType}
                        text={(AppLinkUrl.subdomain() && Auth.subdomainUser()) ?
                            subdomainuser?.user_fullname
                            :
                            (AppLinkUrl.privateDomain() && Auth.user()) ?
                                user?.user_fullname
                                : ""
                ...
                {
                    width < 768 && ResponsiveBooking ?

                        FinalData?.show_Booking_Appointment ?
                            <ResponsiveBookingButton style={responsiveBookingAppointmentButton} onClickHandle={AppointmentRedirect} /> : ""
                        : ""
                }
            </UserProfile>
                                                                                        ...
                                                                                        <MenuSubSubListItem key={subsubcategory._id} item={styles?.MenuSubSubListItem}>
                                                                                            <MenuSubSubListContent item={styles?.MenuSubSubListContent}>
...

                                                                                                {
                                                                                                    ...
                                                                                                    <Button text={subsubcategory?.page?.page_name} style={styles?.MenuSubListContentBtn}
                                                                                                        className={`${value?.page?.page_slug === pathname || (findLastIndex === '/' && value?.page?.page_type === 'home') ? 'active' : ''}`}
                                                                                                    >
                                                                                                        +
                                                                                                    </Button>
