...
import { MenuList, MenuListItem, MenuListItemInside, MenuListSubItem, Navbar } from './nav3.style'
...
import HeaderDropDown3 from '../HeaderDropDown3';
import MoreDropDown3 from '../MoreDropDown3/MoreDropDown3';
import { useDetectOutsideClick } from '@/CommonComponents/DetectOutsideClick/useDetectOutsideClick';
import { useRouter } from 'next/router';
import ValidationFile from '@/Classes/ValidationFile';
const NavBar3 = ({ NavigationData, NavStyle, className }) => {

  const styles = useContext(UserContext3)
                          ...
                          <MenuListItem key={value._id} item={style?.MenuListItem} itemActive={style?.MenuListItemActive}>
                            ...
                            </MenuListItemInside>
                            {
                              value.data && value.data.length > 0 ? (
                                <HeaderDropDown3 dorpDownData={value.data} dropDownStyle={NavStyle?.navigation_menu?.navigationDropDownTypography} />
                              ) : ("")
                            }
          ...
          NavigationData && NavigationData?.data && NavigationData.data.length > 0 && NavigationData.data.slice(3).length ? (
            <>
              <MenuListItem item={style?.MenuListItem} ref={ref}>
                ...
                {
                  more && <MoreDropDown3 DropDownData={NavigationData.data.slice(3)}
