...
import FullSearchBar4 from './FullSearchBar4';
import { BookIcon, HeaderButtonWrapper, MainHeader, SearchCommonIcon } from './headerVariation4.style';
import NavBar4 from './NavBarVariation4';
import HeaderLogo4 from './HeaderLogo4';
import SearchBar4 from './SearchBar4';
import DynamicCommonIcon4 from './DynamicCommonIcon4';
import AppLinkUrl from '@/CommonComponents/AppLink/AppLinkUrl';
import Auth from '@/Classes/Auth';
import BookAppointment4 from './BookAppointment';
export const UserContext4 = createContext();

const HeaderVariation4 = ({ children }) => {
    ...
    console.log(FinalData, "button")
    return (
        <>
            {
                width > 1024 ?
                    <UserContext4.Provider value={dataa}>
                        {
                            searchBar ? <FullSearchBar4 FullSearch={FullSearch} style={style}
                                    ...
                                    <HeaderButtonWrapper>

                                        {
                                            FinalData?.logoAndSiteName?.show_site_logo || FinalData?.logoAndSiteName?.show_site_name ? (
                                                <HeaderLogo4 logoData={FinalData?.logoAndSiteName} className={"centerdLogo"} />
                                            ) : (
                                                ""
                                            )
                                        }
                                        {
                                            FinalData?.show_Booking_Appointment ?
                                                <BookAppointment4 style={bookingAppointmentButton} onClickHandle={AppointmentRedirect} /> : ""
                                        }
                                    </HeaderButtonWrapper>
                                    <SearchCommonIcon>
                                        {
                                            FinalData?.navigation_menu?.hide_navigation ? "" :
                                                <NavBar4 NavigationData={NavigationData} NavStyle={FinalData} className={"centeredNav"} />
                                        }
                                        ...
                                        <BookIcon item={style?.BookIcon}>
                                            ...
                                            <DynamicCommonIcon4 FullSearch={FullSearch} classname={style.leftIcon}
...
export default HeaderVariation4
