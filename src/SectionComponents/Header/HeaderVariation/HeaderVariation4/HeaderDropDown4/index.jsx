...
import { MainHeaderDropdown, MenuDropDownIcon, MenuDropDownList, MenuHeading, MenuSubDropDown, MenuSubHeader, SubLink, SubSubLink, SubSubLinkSection } from './dropDown4.style';
...
const HeaderDropDown4 = ({ dorpDownData, dropDownStyle }) => {
  ...
  const RedirectPage = (slug, prefix) => {
    switch (prefix) {
      case "category":
              ...
              <MenuSubDropDown key={value?._id} item={style?.MenuSubDropDown}>
                <MenuSubHeader item={style?.MenuSubHeader} itemDropDownIcon={style?.DownArrowIcon}>
                  <Button text={value?.page?.page_name}
                    style={CategoryHeadingButton}
                    onClick={() => RedirectPage(value?.page.page_slug, value?.page.prefix)}
                        ...
                        return (
                          <>
                            <SubLink key={subkey} item={style?.SubLink}
                              ...
                              <Button text={item?.page?.page_name} className={item?.page?.page_slug === pathname ? 'active' : ''} style={SubLinkBtn} onClick={() => RedirectPage(item?.page?.page_slug, item?.page?.prefix)} />
                              {
