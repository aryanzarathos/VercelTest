...
import { MoreContainer, MoreList, MoreListItem, MoreSubLink, MoreSubLinkSection, MoreSubListItem } from './MoreDropDown4.style'
...
import { SubSubLinkSection } from '../HeaderDropDown4/dropDown4.style';
import { useRouter } from 'next/router';
import ValidationFile from '@/Classes/ValidationFile';

const MoreDropDown4 = ({ DropDownData, dropDownStyle }) => {
                    ...
                    selecetedList === "1" ? (
                        <MoreList item={style?.MoreList}>
                            <MoreSubListItem className='border' item={style?.MoreSubListItem}
                                ...
                                <Button
                                    onClick={() => RedirectTo(dropDownData?.page?.page_slug, dropDownData?.page?.prefix, dropDownData?.page)}
                                                    ...
                                                    (subItemExpand === item._id) && item.data && item.data.length > 0 && item.data.map((subItem, subSubKey) => (
                                                        <>
                                                            <SubSubLinkSection item={style?.SubSubLinkSection} subSubKey={subSubKey}>
                                                                <Button text={subItem?.page.page_name} className={item?.page?.page_slug === pathname ? 'active' : ''} style={SubSubLinkSectionBtn}
                                                                    onClick={() => RedirectTo(subItem?.page?.page_slug, subItem?.page?.prefix, subItem?.page)} />
                                                            </SubSubLinkSection>
                                                        </>
                                                    ))
                                                }
                                ...
                                <>
                                    {
                                        DropDownData.map((item, key) => {
                                            return (
                                                <MoreListItem key={key} item={style?.MoreListItem} itemDownSvg={style?.DownIcon}>
                                                    <Button className={item?.page?.page_slug === pathname ? 'active' : ''} onClick={() => RedirectTo(item?.page?.page_slug, item?.page?.prefix, item?.page)} style={MoreSubListItemBtn}>
                                                        {item?.page?.page_name}
                                                    </Button>
                                                    {
                                                        item?.data && item.data.length > 0 ? (
